...
class APITest(object):
    ...
    def __init__(self, driver):
        ...
        self.response1 = requests.get(url="https://api.covid19india.org/state_district_wise.json")
        self.response_code1 = self.response.status_code
    ...
    def get_district_data(self, state_name):
        if self.response_code1 == 200:
            self.response_json1 = self.response1.json()
            state_name1 = self.response_json1
            for state_name in state_name1:
                for state1 in state_name:
                    if state1 in self.response_json1.keys():
                        print(state1, "data in district wise are:", self.response_json1[state1])
